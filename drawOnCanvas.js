window.onload = function () {
  ...
  canvas.onmouseup = function(e) {
    ...
    //  Testing purpose use the following code
    tracker = [];
...
function Predict (data) {
  this.calPdf = function (x, mean, stdev) {
  ...
  this.calClassPdf = function (ftrs, aClassMean, aClassStdev) {
    var cumm = 1;
    var idxFtr = 0;
    for (var k in aClassMean) {
      if (k !== "Class" && aClassMean.hasOwnProperty (k) &&
                           aClassStdev.hasOwnProperty (k)) {
        // alert (k + " -> " + aClassMean[k] + " : " + aClassStdev[k]);
        cumm *= this.calPdf (ftrs[idxFtr], aClassMean[k], aClassStdev[k]);
        // alert (cumm);
        idxFtr += 1;
      }
    }
  ...
  var jsonMeans = JSON.parse(dataMeans);
  var jsonSds = JSON.parse(dataSds);
  alert ("Predict " + jsonMeans[0].xLen + " : " + jsonSds[0].xLen);
  // alert ("pdf: " + this.calPdf (71.5, 73, 6.2)); //0.06248965759370005
  // alert ("jsonMeans length " + jsonMeans.length);
  for (var c=0; c<jsonMeans.length; ++c) {
    this.calClassPdf (data, jsonMeans[c], jsonSds[c]);
    break;
  }
}
...
function Recognize (data, width, height, normToSize) {
  ...
  var dataFeat = new FeatureExtract (dataProc);
  var dataProb = new Predict (dataFeat);
